"""TTS inference script for T5Gemma-TTS using 4-bit quantization (GPU).

This script loads the model in 4-bit using bitsandbytes for memory efficiency
and runs inference on the GPU.
"""
...
def run_inference(
    ...
    # Model is already on GPU due to device_map="auto" and load_in_4bit=True
    model.eval()
    cfg = model.config
    ...
    audio_tokenizer = AudioTokenizer(
        backend="xcodec2",
        model_name=getattr(cfg, "xcodec2_model_name", "xcodec2"), # Fallback if not in config
    )
    ...
    if no_reference_audio:
        ...
        # Transcribe audio for reference (optional, requires Whisper)
    ...
    out_path = os.path.join(output_dir, "generated_4bit.wav")
    save_audio(out_path, gen_audio, codec_audio_sr)

    # Calculate some stats to ensure audio isn't silence
