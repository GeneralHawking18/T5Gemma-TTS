"""Debug encoder differences between ONNX and PyTorch."""
import os
with open(".env") as f:
    for line in f:
        if "=" in line:
...
test_texts = [
    ...
    "こんにちは、今日はいい天気ですね",
    "Hello world",
    "This is a longer sentence to test the encoder",
]
...
for text in test_texts:
    ...
    with torch.no_grad():
        ...
        print(f"Position IDs: {position_ids[0, :5].tolist()}...")
...
# Now let's check if the ONNX encoder is computing position IDs correctly
print("\n" + "="*60)
print("INVESTIGATING ONNX POSITION ID COMPUTATION")
print("="*60)

# Test with the same sequence length as was used during ONNX export (64 tokens)
test_ids = tokenizer.encode("This is a test to check position IDs", add_special_tokens=False)
# Pad to 64 tokens
test_ids = test_ids + [tokenizer.pad_token_id or 0] * (64 - len(test_ids))
test_ids = test_ids[:64]

input_ids = torch.tensor([test_ids], dtype=torch.long)
attention_mask = torch.ones_like(input_ids)
# Set padding to 0 in attention mask
actual_len = len(tokenizer.encode("This is a test to check position IDs", add_special_tokens=False))
attention_mask[0, actual_len:] = 0

print(f"\nTest with {actual_len} actual tokens padded to 64")
print(f"Attention mask: {attention_mask[0, :10].tolist()}... sum={attention_mask.sum().item()}")
...
with torch.no_grad():
    ...
    print(f"PyTorch Position IDs (first 10): {position_ids[0, :10].tolist()}")
    print(f"PyTorch Position IDs (last active): {position_ids[0, actual_len-3:actual_len+2].tolist()}")
...
print(f"\nONNX:    mean={onnx_out.mean():.4f}, std={onnx_out.std():.4f}")
print(f"PyTorch: mean={pt_out.mean():.4f}, std={pt_out.std():.4f}")
print(f"Max diff: {diff.max():.4f}, Mean diff: {diff.mean():.4f}")
print(f"Cosine similarity: {cos_sim:.6f}")
